...
Output:
    gpt2_architecture.png  (written next to this script)
"""
...
def build_gpt2():
    acc = "#2b7bba"; tint = "#bcd6ea"; tint_ec = "#7fa8c9"
    rows = [
        {"k": "linout", "t": "Linear output layer"},
        {"k": "fnorm", "t": "Final LayerNorm"},
        {"k": "plus2", "kind": "plus"},
        {"k": "drop2", "t": "Dropout"},
        {"k": "ff", "t": "Feed forward"},
        {"k": "ln2", "t": "LayerNorm 2"},
        {"k": "plus1", "kind": "plus"},
        {"k": "drop1", "t": "Dropout"},
        {"k": "mha", "kind": "attn", "t": "Masked multi-head\nattention"},
        {"k": "ln1", "t": "LayerNorm 1"},
        {"k": "dropin", "t": "Dropout"},
        {"k": "posemb", "t": "Positional embedding layer", "fs": 10.3},
        {"k": "tokemb", "t": "Token embedding layer"},
    ]
    gap = 0.4
    ch = col_height(rows, gap); top = BOT + ch; H = top + 2.9; W = 15; cx = 5.3
    fig, ax = new_ax(W, H)
    col = Column(ax, cx, top, gap); col.place(rows, BW); col.connect()
    outer_and_tint(ax, cx, col, "plus2", "ln1", 6.6, tint, tint_ec)
    title(ax, cx, H - 0.9, ["GPT-2 (124M)"], "#2f6f4f")
    ax.text(cx, H - 1.9, "decoder-only transformer", ha="center", fontsize=12.5, style="italic", color="#555")
    ax.text(cx - 2.55, col.top_of("linout") + 0.05, "GPT model", ha="left", fontsize=11.5, fontweight="bold", color="#333")
    tt_y = col.bot_of("tokemb") - 0.95
    box(ax, cx, tt_y, 2.7, 0.82, "Tokenized text", fc="white")
    arrow(ax, cx, tt_y + 0.41, cx, col.bot_of("tokemb"))
    ax.text(cx, tt_y - 0.9, "Every effort moves you", ha="center", fontsize=12, family="monospace")
    arrow(ax, cx, col.top_of("linout"), cx, col.top_of("linout") + 0.85)
    lx = cx - 2.0
    residual(ax, cx, col, "plus1", "ln1", lx, gap)
    residual(ax, cx, col, "plus2", "ln2", lx, gap)
    curly_brace(ax, cx, col, "plus2", "ln1", "12 ×", tint_ec)
    axr = cx + 3.85
    leader(ax, cx + BW / 2, col.pos["linout"], axr - 0.3, col.pos["linout"] + 0.55)
    rich(ax, axr, col.pos["linout"] + 0.55, [[("Vocabulary size of ", "black", "bold")], [("50,257", acc, "bold")]])
    leader(ax, cx + BW / 2, col.pos["tokemb"], axr - 0.3, col.pos["tokemb"] - 0.3)
    rich(ax, axr, col.pos["tokemb"] - 0.3, [[("Embedding", "black", "bold")],
         [("dimension of ", "black", "bold"), ("768", acc, "bold")]])
    leader(ax, cx + BW / 2, col.pos["mha"], axr - 0.3, col.pos["mha"])
    rich(ax, axr, col.pos["mha"], [[("12", acc, "bold"), (" attention heads", "black", "bold")],
         [("Context length ", "black", "bold"), ("1,024", acc, "bold")],
         [("GELU feed-forward (4×)", "black", "normal")]], fs=11.5)
    finish(fig, "gpt2_architecture")
